...
fun build(service: Service, req: Request): Response {
  val gameName = req.pathGameName()
    ?: return badRequest("Must specify the name of the game: /api/v1/games/{game_name}/build")

  val game = service.getGame(gameName)
    ?: return notFound("The game '$gameName' was not found.")

  val turn = Body.auto<BuildTurnDTO>().toLens().extract(req)
