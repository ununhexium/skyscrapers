...
data class GameState(
      ...
      val positions =
        lines.drop(1).takeWhile { !it.startsWith("Blocks:") }.map { line ->
          line.trim().split(Regex(" +")).map { position ->
            net.lab0.skyscrapers.engine.structure.CompositePosition.from(
              position
            )
          }
        }
  ...
  fun toCompositeString(): String {
    val fullState =
      buildings.merge(seals.merge(builders) { it }) {
        net.lab0.skyscrapers.engine.structure.CompositePosition(
          it.first.value,
          it.second.first,
          it.second.second
        )
  ...
  val phase: net.lab0.skyscrapers.engine.structure.Phase
    get() =
      if (isFinished()) {
        net.lab0.skyscrapers.engine.structure.Phase.FINISHED
      } else {
        val totalBuilders = players.size * maxBuildersPerPlayer
        val placedBuilders = builders.count { it != null }
        if (placedBuilders < totalBuilders) net.lab0.skyscrapers.engine.structure.Phase.PLACEMENT else net.lab0.skyscrapers.engine.structure.Phase.MOVEMENT
      }
